...
export function DataProvider({ children }) {
  ...
  useEffect(() => {
    const getUserData = async () => {
      const getUserDataEndPoint = `${apiDomain}/api/${apiVersion}/users/${localStorage.getItem(
        "user_id"
      )}`;
      await axiosInstance.get(getUserDataEndPoint).then((response) => {
    ...
    const getUserHouseholdData = async (householdId) => {
      if (householdId !== null) {
        ...
        await axiosInstance
          .get(getUserHouseholdDataEndPoint)
          .then((response) => {
            if (isMounted.current) {
    ...
    const getUserOptionData = async (userId) => {
      ...
      await axiosInstance.get(getUserOptionDataEndPoint).then((response) => {
        if (isMounted.current) {
    ...
    const fetchNotification = async (userId) => {
      ...
      await axiosInstance.get(getNotificationEndPoint).then((response) => {
        if (isMounted.current) {
    ...
    const connectSocketIo = () => {
      ...
      socketRef.current.on("connect", () => {
        socketRef.current.emit("enterSocketRoom", {
          socketRoomName: localStorage.getItem("user_id"),
        });
      });
      ...
      socketRef.current.on("deleteNotificationReceived", (notifId) => {
        setNotificationReceived((notificationReceived) =>
          notificationReceived.filter((notif) => notif._id !== notifId)
        );
      });

      socketRef.current.on(
        "updateAllNotificationsReceived",
        (allNotifReceived) => {
  ...
  return (
    <UserDataContext.Provider value={{ userData, setUserData }}>
      <UserHouseholdDataContext.Provider
        value={{ userHouseholdData, setUserHouseholdData }}
      >
        <UserOptionContext.Provider
          value={{ userOptionData, setUserOptionData }}
        >
